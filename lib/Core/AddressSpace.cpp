...
ObjectState *AddressSpace::getWriteable(const MemoryObject *mo,
  ...
  ref<ObjectState> newState(new ObjectState(*os));
  newState->copyOnWriteOwner = cowKey;
  objects = objects.replace(std::make_pair(mo, newState));
  return newState.get();
}
...
bool AddressSpace::resolveOne(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {
    ...
    if (!solver->getValue(state.constraints, address, cex,
...
bool AddressSpace::resolve(ExecutionState &state, TimingSolver *solver,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(p)) {
    ...
    ref<ConstantExpr> cex;
    if (!solver->getValue(state.constraints, p, cex, state.solverMetaData))
      return true;
    uint64_t example = cex->getZExtValue();
    MemoryObject hack(example);
