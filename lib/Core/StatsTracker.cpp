...
void StatsTracker::stepInstruction(ExecutionState &es) {
  if (OutputIStats) {
	...
	// Only generate a test case if it is the beginning of a basic block and
	// the some query costs were generated by the state which implies
	// that it dependet on some symbolic values
       if(&*inst->getParent()->begin() == inst && !inst->getParent()->getSinglePredecessor()  &&
		       es.queryCost > klee::time::Span()) {
