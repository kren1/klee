...
namespace {
  ...
  CoreSearch("search", cl::desc("Specify the search heuristic (default=random-path interleaved with nurs:covnew)"),
	     cl::values(clEnumValN(Searcher::DFS, "dfs", "use Depth First Search (DFS)"),
			...
			clEnumValN(Searcher::Profile, "prof", "profile based searcher"),
			clEnumValN(Searcher::InverseProfile, "inv-prof", "profile based searcher"),
...
Searcher *getNewSearcher(Searcher::CoreSearchType type, Executor &executor) {
  ...
  case Searcher::Profile: searcher = new ProfileSearcher(executor); break;
  case Searcher::InverseProfile: searcher = new InverseProfileSearcher(executor); break;
