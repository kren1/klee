...
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --debug-print-instructions=all:stderr %t2.bc 2>%t3.txt
...
// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --debug-print-instructions=all:file %t2.bc
...
// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --debug-print-instructions=src:file %t2.bc
// RUN: FileCheck -input-file=%t.klee-out/instructions.txt -check-prefix=CHECK-FILE-SRC %s
// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --debug-print-instructions=compact:file %t2.bc
