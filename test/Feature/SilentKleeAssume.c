...
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --silent-klee-assume %t.bc > %t.silent-klee-assume.log 2>&1
// RUN: FileCheck -input-file=%t.silent-klee-assume.log -check-prefix=CHECK-SILENT-KLEE-ASSUME %s
// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All %t.bc > %t.default-klee-assume.log 2>&1
