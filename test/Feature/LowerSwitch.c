...
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --external-calls=all --switch-type=internal %t.bc
// RUN: not test -f %t.klee-out/test000010.ktest

// RUN: rm -rf %t.klee-out
// RUN: %klee --output-dir=%t.klee-out --exit-on-error-type=All --switch-type=simple %t.bc
